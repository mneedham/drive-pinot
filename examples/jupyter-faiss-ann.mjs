...
async function run() {
    const zoomLevel = 400;
    const token = "0761eaffb407c7784cce43ae87b2ab48ec2aed24ea5b058a";
    const { browser, page } = await launchBrowseAndOpenPage({
        url: `http://localhost:8888/doc/tree/ANN-Tutorial.ipynb?token=${token}`, 
        zoomLevel: `${zoomLevel}%`
    });
    ...
    code = `!pip install faiss-cpu pandas numpy`;
